...
const float GRAN = 0.25; //0.25; // in meters

// threshold at which we move to the next point in the path
const float CARROT_RADIUS = 2.0; 
const float STOP_DIST = 0.2;
const float REWARD_MIN_FREE_PATH = 0.3;
...
void Navigation::SetNavGoal(const Vector2f& loc, float angle) {
  cout << "In SetNavGoal" << endl;
  global_planner_.UpdateGoal(loc(0), loc(1));
  if (!map_.lines.empty()) {
    cout << "start a* search." << endl;
    plan_success_ = global_planner_.Search(robot_loc_(0), robot_loc_(1), global_path_);
...
void Navigation::UpdateLocation(const Eigen::Vector2f& loc, float angle) {
  ...
  if (i == 5) {
    global_path_.clear();
    global_planner_.Search(robot_loc_(0), robot_loc_(1), global_path_);
    path_index_ = 5;
    i = 0;
  }
}

void Navigation::Initialize(const std::string& map_file,
...
void Navigation::RunAutonomous() {
  ...
  last_curv_ = chosen_path.curvature;
...
void Navigation::PublishVis() {
  ...
  //   last_curv_ = 0.001;
  //   // cout << "here" << endl;
  //   // visualization::DrawLine(Vector2f(0, 0), Vector2f(3, 0), 0x0000ff, local_viz_msg_);
  // }
  //   visualization::DrawArc(Vector2f(0, 1.0 / last_curv_), 1.0 / last_curv_, 0, M_PI * 2, 0x0000ff, local_viz_msg_);

  if (plan_success_) {
    visualization::DrawCross(global_path_[path_index_], 0.4, 0x0000ff, global_viz_msg_);
  }

  // draw global path
  // cout << "publishing " << global_path_.size() << " points" << endl;
  for (size_t i = 1; i < global_path_.size(); i++) {
    visualization::DrawLine(global_path_[i-1], global_path_[i], 0x0000ff, global_viz_msg_);
  }
...
float Navigation::ComputeCarrotDistance(float proposed_curvature) {
  Vector2f path_point = global_path_[path_index_];
  // Vector2f path_point = global_path_.back();

  // translate the path point into the frame of the car 
  // to make some of the math easier
  // rotate by car angle

  Affine2f global_to_robot = DeltaToTransform(robot_loc_(0), robot_loc_(1), robot_angle_).inverse();

  Vector3f goal_point_v(path_point(0), path_point(1), 1);

  Vector3f trans_goal = global_to_robot * goal_point_v;

  float x_prime = trans_goal(0);
  float y_prime = trans_goal(1);

  if (proposed_curvature == 0) {
    // since we have transformed into the frame of the car, 
    // the y coordinate of the path point gives us the distance 
    // of the path point from the curve the car will follow
    return y_prime;
  }

  // find the turning radius of the proposed turn
  ...
  // turning_center_x is always zero because we are in the frame of the car

  float dist_to_point = std::sqrt(std::pow(turning_center_x - x_prime, 2) + 
    std::pow(turning_center_y - y_prime, 2)) - turning_radius;

  return dist_to_point;
}

void Navigation::DoPointUpdate(void) {
  Vector2f path_point = global_path_[path_index_];
  // get distance of car to the current path point
  float x_diff = robot_loc_(0) - path_point(0);
  float y_diff = robot_loc_(1) - path_point(1);

  float distance = hypot(x_diff, y_diff);

  if (distance <= CARROT_RADIUS && path_index_ < global_path_.size() - 1) {
    path_index_++;
  }
}

float Navigation::DistToGoal() {
  return std::hypot(robot_loc_(0) - global_path_.back()(0), robot_loc_(1) - global_path_.back()(1));
}
...
std::pair<PathOption, float> Navigation::PickCurve(vector<float> proposed_curves) {
  ...
  for (auto curve : proposed_curves) {
    ...
    // if we currently have a global plan, add a term to account for how close each curve is 
    // to the next point in the plan
    if (plan_success_) {
      // compute carrot distance
      new_option.carrot_distance = ComputeCarrotDistance(curve);
    }
...
PathOption Navigation::RewardFunction(vector<PathOption> path_options) {
  ...
  for (auto option : path_options) {
    ...
    cout << "curv: " << option.curvature << "\t clear: " << option.clearance << " \tcarrot: " << option.carrot_distance << "\tpathlen: " << option.free_path_length << endl;
...
float Navigation::ApplyRewardFunction(PathOption option) {
  ...
  if (option.free_path_length < REWARD_MIN_FREE_PATH) {
    return 0;
  }

  return (5.0 * option.free_path_length) + (-0.05 * std::abs(option.curvature)) + (5.0 * std::max(option.clearance, (float)1.0)) + (1.5 * (1.0 / option.carrot_distance));
  // return (100 * (1 / option.carrot_distance));
...
double Navigation::CalculateClearance(float proposed_curvature) {
  ...
  // TODO: what happens if the point cloud point is within our turning radius?
  for(auto point : point_cloud_) {
    float dist_to_point = std::sqrt(std::pow(turning_center_x - point(0), 2) + std::pow(turning_center_y - point(1), 2));
    new_min = dist_to_point - turning_radius;
    if (new_min < 0) {
      new_min = turning_radius - dist_to_point;
    }
    if (new_min < global_min) {
      global_min = new_min;
    }
