...
uint32_t PickColor(float weight, float weight_sum) {
  // uint32_t base_color = 0xc300ff;
  float normalized_weight = weight / weight_sum;
  // cout << normalized_weight << endl;
  if (normalized_weight > 0 && normalized_weight < 0.01) {
    return 0xff0000; // red
  }
  else if (normalized_weight >= 0.01 && normalized_weight < 0.02) {
    return 0xff8800; // orange 
  } else if (normalized_weight >= 0.02 && normalized_weight < 0.03) {
    return 0xbfff00; // yellow green
  } else if (normalized_weight >= 0.03 && normalized_weight < 0.04) {
    return 0xfffb00; // yellow 
  } else {
    return 0x2fff00; // green
  }
  return (uint32_t)normalized_weight;
}
...
void PublishParticles() {
  ...
  for (const particle_filter::Particle& p : particles) {
    // std::cout << "drawing particle at " << p.loc(0) << ", " << p.loc(1) << std::endl;
    DrawParticle(p.loc, p.angle, p.weight, vis_msg_);
    visualization::DrawCross(p.loc, 0.25, PickColor(p.weight, weight_sum), vis_msg_);
  }
}

void PublishPredictedScan() {
  const uint32_t kColor = 0x282ca1;
